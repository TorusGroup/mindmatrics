...
const DigitalAddictionReport: React.FC<DigitalAddictionReportProps> = ({ data, lead, onBookingRequest }) => {
  const { t } = useI18n();
  
  if (!data) return null;

  const getArchetype = (score: number) => {
      if (score > 75) return t('digital_report.archetypes.escapist');
      if (score > 50) return t('digital_report.archetypes.scroller');
      return t('digital_report.archetypes.checker');
  };
  ...
  return (
    <ReportWrapper 
      lead={lead} 
      reportTitle={t('digital_report.title')} 
      reportSubtitle={t('digital_report.titleAccent')}
    ...
    >
      <section className="p-12 md:p-24 bg-white border-b border-gray-100">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              <div>
                  <h3 className="font-serif text-3xl mb-6">{t('digital_report.life_drain')}</h3>
                  <div className="p-8 bg-offwhite rounded-3xl border border-gray-100">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{t('digital_report.total_consumed')}</p>
                      <h4 className="text-6xl font-serif text-darkgray font-bold">{totalYearsLost} <span className="text-2xl font-sans text-gray-300">{t('digital_report.years')}</span></h4>
      ...
      <section className="p-12 md:p-24 bg-offwhite">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100">
                  <h4 className="font-bold text-red-600 uppercase tracking-widest text-[10px] mb-8">{t('digital_report.current_state')}</h4>
              ...
              <div className="flex flex-col justify-center">
                  <h3 className="font-serif text-3xl mb-4">{t('digital_report.price_of_distraction')}</h3>
                  <p className="text-gray-500 text-sm leading-relaxed mb-8">{t('investment.roiDesc')}</p>
